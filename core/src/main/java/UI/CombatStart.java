...
public class CombatStart {
	...
	public static void updateHp(Label playerHp, Player player, Label enemyHp, Entity enemy) {
	...
	public CombatStart(Player player, Entity enemy,Random rand) {
		...
		Kick.setOnAction(e -> {
			player.kick(rand, enemy, yapp);
			enemy.taketurn(player, rand, yapp);
			updateHp(playerHp,player, enemyHp, enemy);
		});
		
		Button SwordSlash = new Button("SwordSlash");
		SwordSlash.setOnAction(e -> {
			player.swordSlash(rand, enemy, yapp);
			enemy.taketurn(player, rand, yapp);
			updateHp(playerHp, player, enemyHp, enemy);
		});
        ...
        VBox HpDisplay = new VBox(20);
        HpDisplay.setAlignment(Pos.BOTTOM_RIGHT);
        HpDisplay.getChildren().addAll(playerHp, enemyHp);
        
        BorderPane mainLayout = new BorderPane();
        mainLayout.setLeft(root);
        mainLayout.setRight(HpDisplay);
        mainLayout.setCenter(yapp);
